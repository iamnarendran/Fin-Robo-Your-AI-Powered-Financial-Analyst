...
# ---------- TAB 2 ----------
    ...
    # --- CSS Styling ---
        ...
        if "stock_data_dict" not in st.session_state or not st.session_state["stock_data_dict"]:
            st.warning("Please first select and plot some stocks in the Charts tab")
        if "api_key" not in st.session_state or not st.session_state["api_key"]:
            st.warning("⚠️ Please enter your OpenRouter API key in the sidebar to enable AI insights.")
        else:
            st.markdown("You can ask anything about the selected stocks. For example:")
            st.markdown("- *Compare Tesla and Apple performance this year*")
            ...
            if user_query:
                ...
                with st.spinner("Thinking..."):
                    ...
                    try:
                        response_text = analyze_stocks_with_ai_prompt(full_prompt,api_key, model_name)
